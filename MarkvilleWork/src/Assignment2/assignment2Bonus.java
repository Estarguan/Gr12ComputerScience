...
public class assignment2Bonus {
    ...
    public static int dfs(char[][] grid, int minSteps, int steps, int r, int c, int[][] dir,int row, int col,boolean[][] visited, String curPath,char target) {
//        System.out.println(curPath);
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (grid[tempR][tempC] == 'X') {
                continue;
            }if (grid[tempR][tempC] == '-' || grid[tempR][tempC] == target || grid[tempR][tempC] == 'L' || grid[tempR][tempC] == 'W') {
                if (i == 0)curPath+="S ";
                if (i==1)curPath+="N ";
                if (i==2)curPath+="E ";
                if (i==3)curPath+="W ";
                visited[tempR][tempC] = true;
                minSteps =  dfs(grid,minSteps,steps+1,tempR,tempC,dir,row,col,visited,curPath,target);
    ...
    public static void main(String[] args)throws IOException {
        ...
        BufferedReader in = new BufferedReader(new FileReader("assignment2Bonus.txt"));
        ...
        for (int z = 0; z < testCases; z++) {
            ...
            for (int i = 0; i < row; i++) {
                ...
                for (int j = 0; j < col; j++) {
                    ...
                    visited[i][j] = false;
                    if (line.charAt(j) == 'L')hasLitter = true;
                }
            }
            visited[0][0] = true;
            ...
            int steps = dfs(grid,Integer.MAX_VALUE,0,0,0,dir,row,col,visited, "",'S');
            //If there is a litterbox which there defininetly should be for all bonus cases then dfs again
            visited[row - 1][col - 1] = true;
            int litterSteps = Integer.MAX_VALUE;
            visited[0][0] = false;
            if (hasLitter) litterSteps = dfs(grid, Integer.MAX_VALUE, 0, row - 1, col - 1, dir, row, col, visited, minPath, 'L');
            if (!hasLitter && steps != Integer.MAX_VALUE){
                System.out.println("Fastest # of steps: " + (steps));
                System.out.println("Direction: " + minPath);
            }
            else if (steps == Integer.MAX_VALUE || litterSteps == Integer.MAX_VALUE) {
                System.out.println("No path");
            }else {
                System.out.println("Fastest # of steps: " + (steps+litterSteps));
                System.out.println("Direction: " + minPath);
            }
            System.out.println("");
